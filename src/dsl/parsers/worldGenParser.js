import {Parser, tokenMatcher, exceptions as chevrotainExceptions} from 'chevrotain';
...
import {WorldGenConfiguration, BeastEndYear, MinMaxVariance, MeshFrequency, TitanAttackTrigger, SquareCounts, MinimumSquareCounts, RiverMins, MinimumLowMedHighSquares} from './../../model/worldGen';
...
export default class WorldGenParser extends Parser {
	constructor(tokens, tokenConstructors = []) {
		const filteredTokens = _.reject(tokens, (token) => tokenMatcher(token, Comment));
		super(filteredTokens, tokenConstructors);

		this.RULE('parseWorldGenFile', () => {
			const worldGenConfigurations = [];
			this.MANY(() => {
				const worldGenConfiguration = this.SUBRULE(this.worldGenConfiguration);
		...
		this.worldGenConfiguration = this.RULE('worldGenConfiguration', () => {
			...
			this.MANY({
				GATE() {
					...
					return tokenMatcher(nextNameToken, worldGenTokens.WorldGenSettingToken);
				},
				DEF: () => {
			...
			const rawWorldGenConfiguration = _.transform(tokensByName, (rawConf, dfTokensAtName, name) => {
				...
				switch (name) {
					case 'TITLE':
						...
						break;
					case 'DIM':
						_.assign(rawConf, {[propName]: {x: _.parseInt(tagArgs[0]), y: _.parseInt(tagArgs[1])}});
						break;
					// integers
					case 'EMBARK_POINTS':
					case 'END_YEAR':
					...
					case 'SITE_CAP':
						_.assign(rawConf, {[propName]: _.parseInt(tagArgs[0])});
						break;
					case 'BEAST_END_YEAR':
						_.assign(rawConf, {[propName]: new BeastEndYear({startYear: _.parseInt(tagArgs[0]), percentage: _.parseInt(tagArgs[1])})});
						break;
					// booleans
					...
					case 'VOLCANISM':
					case 'SAVAGERY':
						_.assign(rawConf, {[propName]: new MinMaxVariance({min: _.parseInt(tagArgs[0]), max: _.parseInt(tagArgs[1]), varianceX: _.parseInt(tagArgs[2]), varianceY: _.parseInt(tagArgs[3])})});
						break;
					...
					case 'VOLCANISM_FREQUENCY':
						_.assign(rawConf, {[propName]: new MeshFrequency({mesh: _.parseInt(tagArgs[0]), weights: _.map(_.tail(tagArgs), _.parseInt)})});
						break;
					case 'POLE':
						_.assign(rawConf, {[propName]: tagArgs[0]});
						break;
					case 'TITAN_ATTACK_TRIGGER':
						_.assign(rawConf, {[propName]: new TitanAttackTrigger({
					...
					case 'GOOD_SQ_COUNTS':
					case 'EVIL_SQ_COUNTS':
						_.assign(rawConf, {[propName]: new SquareCounts({
						...
						break;
					case 'REGION_COUNTS':
						_.assign(rawConf, {[propName]: _.reduce(dfTokensAtName, (regionCounts, regionCountToken) => {
							const {args: regionCountsTokenArgs} = regionCountToken;
							return _.assign({}, regionCounts, {[regionCountsTokenArgs[0]]: new MinimumSquareCounts({
								initial: _.parseInt(regionCountsTokenArgs[1]),
								initialRegion: _.parseInt(regionCountsTokenArgs[2]),
								final: _.parseInt(regionCountsTokenArgs[3])
							})});
						}, {})});
						break;
					case 'RIVER_MINS':
						_.assign(rawConf, {[propName]: new RiverMins({preErosion: _.parseInt(tagArgs[0]), postErosion: _.parseInt(tagArgs[1])})});
						break;
					...
					case 'VOLCANISM_RANGES':
						_.assign(rawConf, {[propName]: new MinimumLowMedHighSquares({
			...
			try {
				...
				throw this.SAVE_ERROR(new chevrotainExceptions.MismatchedTokenException(err));
			}
		});
