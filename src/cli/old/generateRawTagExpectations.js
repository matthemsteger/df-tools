...
export async function getExpectations(rawFileNames, between) {
	...
	return Promise.map(rawFileNames, async (filePath) => {
		const fileContents = await fs.readFileAsync(filePath, {
			encoding: 'utf8'
		});
		...
		if (!_.isEmpty(between)) {
			const firstItemIdx = _.findIndex(genericLangTokens, [
				'tokenName.image',
				between
			]);
			if (firstItemIdx > -1) {
				const slice = _.slice(genericLangTokens, firstItemIdx + 1);
				genericLangTokens = _.takeWhile(
					slice,
					(token) => token.tokenName.image !== between
				);
			}
		}

		const expectations = _.reduce(
			genericLangTokens,
			(accumulated, token) => {
				...
				if (_.has(accumulated, name)) {
					...
					if (_.get(accumulated, `${name}.numArgs`) !== numArgs) {
						debug(
							'there was already a expectation for %s and it has %d args != %d',
							name,
							_.get(accumulated, `${name}.numArgs`),
							numArgs
						);
						variableArgs = true;
						if (argNumVariations.length === 0) {
							argNumVariations.push(
								_.get(accumulated, `${name}.numArgs`)
							);
						}
...
export async function handler(argv) {
	try {
		...
		await fs.writeFileAsync(
			filename,
			JSON.stringify(expectationsToWrite, null, '\t')
		);
		process.exit(0);
	} catch (err) {
		debug(err);
