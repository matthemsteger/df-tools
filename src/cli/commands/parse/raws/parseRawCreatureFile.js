...
import {parseCreatureRawFile} from './../../../../model/raws/creature/creatureRawProcessing';
...
export async function handler(argv) {
	...
	try {
		...
		const nestedParsedCreatures = await Promise.map(files, async (fileName) => {
			const text = await fs.readFileAsync(fileName, 'utf8');
			const {status, value, index, expected} = rawCreaturesParser.file.parse(text);
			if (!status) {
			...
			return parseCreatureRawFile(value);
		});

		const creatures = R.unnest(nestedParsedCreatures);
		debug('parsed %d creatures', creatures.length);
		await Promise.fromCallback((callback) => process.stdout.write(JSON.stringify(creatures, null, '\t'), 'utf8', callback));
	} catch (err) {
		await Promise.fromCallback((callback) => process.stderr.write(JSON.stringify(serializeError(err)), 'utf8', callback));
		process.exit(1);
	}
}
