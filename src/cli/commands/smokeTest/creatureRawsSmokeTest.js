...
export const describe =
	'Parse all creatures raws and see if there are any errors';
...
export async function handler(argv) {
	try {
		...
		const rawFileNames = await Promise.fromCallback((callback) =>
			glob(
				`${rawsPath}/creature_*.txt`,
				{nodir: true, absolute: true},
				callback
			)
		);
		const rawResults = await Promise.mapSeries(
			rawFileNames,
			async (fileName) => {
				...
				if (parsed.status === false) {
					debug(parsed);
					throw new Error(
						`Creature raw ${fileName} could not be parsed`
					);
				}
			}
		);
