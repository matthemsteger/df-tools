...
import {propsPath, renameKeys, deriveObjFromPaths} from './../../utility/objects';
...
function runDwarfFortressProcess(executablePath, worldGenId, config, cwd) {
	...
	return Future((reject, resolve) => {
		...
		return () => {
			debug('Entered cancel run dwarf fortress with dfProcess %o', dfProcess);
...
function runDwarfFortressGenWorldWithContext(mergeIntoContext, tapFilteredContext) {
	return R.compose(
		R.map(mergeIntoContext),
		R.apply(runDwarfFortressProcess),
		propsPath([['finalExecutablePath'], ['worldGenId'], ['config'], ['install', 'path']]),
		tapFilteredContext(['finalExecutablePath', 'worldGenId', 'config', 'install'])
	);
}
...
function initialInformationWithContext(zipIntoContext, creatures) {
	return R.compose(
		R.map(zipIntoContext(['install', 'worldSettings', 'usedIds', 'rawCreatures'])),
		parallel(4),
		R.juxt([
			discoverInstall,
			getWorldGenSettings,
...
function gameLogSizeWithContext(valueIntoContext, tapFilteredContext) {
	return R.compose(
		R.map(R.compose(valueIntoContext('initialGamelogSize'), R.prop('size'))),
		R.compose(fs.statFuture, R.prop('gamelogPath')),
		tapFilteredContext(['gamelogPath', 'finalExecutablePath', 'worldGenId']),
		R.compose(valueIntoContext('gamelogPath'), gamelogPathFromContext),
		R.compose(valueIntoContext('finalExecutablePath'), finalExecutablePathFromContext),
...
function checkLogForErrorsWithContext(context) {
	...
	return R.compose(
		R.ifElse(
			...
			R.always(rejectedFutureOf(new Error(`Could not find a world generation configuration named ${config}`))),
...
function parseResultsWithContext(zipIntoContext) {
	return R.compose(
		...
		R.converge(both, [
			R.compose(parseWorldSitesAndPops, parseWorldSitesAndPopsArgsFromContext),
...
export function genWorld({dfRootPath, config, creatures, id = null}) {
	...
	const {mergeIntoContext, zipIntoContext, valueIntoContext} = createContext(context);
	const tapFilteredContext = (props) => R.tap(debugFilteredContext(debug, props));
	...
	return R.compose(
		...
		R.chain(runDwarfFortressGenWorldWithContext(mergeIntoContext, tapFilteredContext)),
...
export function genWorlds({dfRootPath, config, numWorlds = 1}) {
	debug('starting genWorlds with dfRootPath:%s, config:%s, numWorlds:%d', dfRootPath, config, numWorlds);
