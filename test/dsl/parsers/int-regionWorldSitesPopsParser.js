...
describe('createRegionWorldSitesPopsParser integration tests', function () {
	...
	[
		...
		it(`should parse properly a world sites file ${path.basename(testFile)}`, async function () {
			const start = Date.now();
			const fakeRegionBuffer = await fs.readFileAsync(path.resolve(__dirname, testFile));
			const fakeRegionText = iconv.decode(fakeRegionBuffer, 'cp437');

			const result = createRegionWorldSitesPopsParser(creatures).file.parse(fakeRegionText);
