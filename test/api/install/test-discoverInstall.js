...
import R from 'ramda';
import {setupModuleDependency, ensureModuleFunctionExport, fakeFuturifiedFs} from './../../helpers';
...
const {maybeDirHasFile: maybeDirHasFileStub, fs: {readFileFuture: readFileFutureStub}} = fsModule;
const pathModule = fakeModule(['resolve']);
const osModule = fakeModule(['type']);
const {type: osTypeStub} = osModule;

const discoverInstallModule = proxyquire('./../../../src/api/install/discoverInstall', {
	path: pathModule,
	os: osModule,
	'./../../utility/fs': fsModule,
...
describe('api/install/discoverInstall', function () {
	...
	describe('discoverInstallMeta', function () {
		...
		it('should get the meta of a df notes file', function (done) {
			maybeDirHasFileStub.returns(R.always(futureOf(Maybe.of('release notes.txt'))));
			readFileFutureStub.returns(futureOf(releaseNotes));
			discoverInstallMeta(installPath).fork(
				done,
				(meta) => {
	...
	describe('discoverInstall', function () {
		...
		beforeEach(function () {
			discoverInstallMetaStub = sandbox.stub(discoverInstallModule, 'discoverInstallMeta');
		});
		...
		it('should return a dwarf fortress install from a dfRootPath', function (done) {
			...
			discoverInstall({dfRootPath}).fork(
				done,
				(install) => {
					expect(install).to.equal(installResult);
					expect(createDwarfFortressInstallStub).to.have.been.calledWithMatch({
						path: dfRootPath,
						version,
						osType
					});
